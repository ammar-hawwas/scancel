...
def intense_scan(ip_range):
    ...
    for host in scanner.all_hosts():
        ...
        for port in scanner[host].all_tcp():
            ...
            results_data.append([host, port, port_data.get('state'), port_data.get('name')])
            print(f"Port: {port}, State: {port_data.get('state')}, Service: {port_data.get('name')}")
        
        all_results.extend(results_data)

    # تحويل النتائج إلى DataFrame
    df = pd.DataFrame(all_results, columns=["IP", "Port", "State", "Service"])
    ...
    csv_filename = "intencse_scan.csv"

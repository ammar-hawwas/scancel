...
def intense_scan_plus_udp(ip_range):
    # إنشاء كائن الماسح الضوئي
    scanner = nmap.PortScanner()
    print(f"Starting intense scan plus UDP for range: {ip_range}")
    
    # تنفيذ الفحص المكثف TCP مع فحص UDP (بدون تعديل في جزء الفحص)
    scanner.scan(ip_range, arguments='-T5 -A -p- -sU')
    ...
    csv_filename = "intense_scan_plus_udp.csv"
...
intense_scan_plus_udp(ip_range)
